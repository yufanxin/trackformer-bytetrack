...
def build_VisDrone(image_set, args): # Edited
    root = Path(args.mot_path) # Attention, still use mot_path, when use, modify train.yaml
    # = /data/wujiapeng/datasets/VisDrone2019/VisDrone2019/
    assert root.exists(), f'provided VisDrone path {root} does not exist'
    
    split = getattr(args, f"{image_set}_split") # When use, modify train.yaml  = args.train_split = train_coco

    img_folder = root / split  # = /data/wujiapeng/datasets/VisDrone2019/VisDrone2019/train_coco
    ann_file = root / f"annotations/{split}.json" # Attention
    #  = /data/wujiapeng/datasets/VisDrone2019/VisDrone2019/annotations/train_coco.json
