...
def evaluate(model, criterion, postprocessors, data_loader, device,
    ...
    base_ds = get_coco_api_from_dataset(data_loader.dataset)  # base_ds: dataset.COCO contains of GTs
    iou_types = tuple(k for k in ('bbox', 'segm') if k in postprocessors.keys())  # only bbox
    coco_evaluator = CocoEvaluator(base_ds, iou_types)
    # coco_evaluator.coco_eval[iou_types[0]].params.iouThrs = [0, 0.1, 0.5, 0.75]

    panoptic_evaluator = None
    if 'panoptic' in postprocessors.keys():  # False
    ...
    if panoptic_res is not None:  # False
    ...
    if args.tracking and args.tracking_eval:
        ...
        # we compute seqs results on multiple nodes but evaluate the accumulated
