...
class BYTETracker_mulcls(object):
    """
    ...
    def add_tracks(self, pos, labels, scores, hs_embeds, masks=None, attention_maps=None):
    ...
    def step(self, blob):
        ...
        if num_prev_track:
            ...
            det_labels1 = det_result['labels'][:-self.num_object_queries]
        ...
        det_labels2 = det_result['labels'][-self.num_object_queries:]
        ...
        det_labels2_high = det_labels2[det_keep2_high]
        ...
        det_labels2_high_new = det_labels2_high[reid_mask2_high]
        ...
        det_labels2_low = det_labels2[det_keep2_low]
        ...
        det_labels_new = torch.cat((det_labels2_high_new, det_labels2_low[reid_mask2_low]), dim=0)
        ...
        for track in self.tracks:  # 计算self.results 作为结果
            ...
            self.results[track.id][self.frame_index]['label'] = track.label.cpu().numpy()  # Edited
...
class Track(object):  # 每个轨迹的类 包含每个轨迹应有的信息
    ...
    def __init__(self, pos, label, score, track_id, hs_embed,
                 mask=None, attention_map=None):
        self.id = track_id
        self.pos = pos

        self.label = label  # Edited
