...
class Tracker:
    ...
    def step(self, blob):
        ...
        if num_prev_track:
            ...
            # print(f"****len of new{len(result['scores'])}****")
            # print(f"****len of tracks and inactive{len(self.tracks)}{len(self.inactive_tracks)}****")
            ...
            track_keep = torch.logical_and(
                ...
                result['labels'][:-self.num_object_queries] == 0)
            print(f"****tracks_len{len(self.tracks)}****inactive_tracks_len{len(self.inactive_tracks)}")
            print(f"****tracks_to_inactive{len(tracks_to_inactive)}")
            # reid queries
        ...
        # print(f"***new_det_boxes{new_det_boxes.shape}")
        if 'masks' in result:
        ...
        print(f"****new_det{new_det_boxes.shape}")
        ...
        # print(f"****final add new track{len(new_det_boxes)}")
        print(f"****new_tracks{new_det_boxes.shape}")
        # final add track
